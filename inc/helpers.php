...
if ( ! function_exists( 'myself_excerpt_length' ) ) :
	/**
	 * long excerpt
	 * 
	 * @since Myself 1.0.0
	 ...
	 */
	function myself_excerpt_length( $length ){
		if ( is_admin() ) {
			return $length;
		}
...
add_filter( 'excerpt_length', 'myself_excerpt_length', 999 );
...
function myself_custom_header_banner_title() {
	...
	elseif ( is_search() ) : ?>
		<h2 class="page-title"><?php printf( esc_html__( 'Search Results for: %s', 'myself' ), get_search_query() ); ?></h2>
	<?php elseif ( is_404() ) :
		echo '<h2 class="page-title">' . esc_html__( 'Oops! That page can&#39;t be found.', 'myself' ) . '</h2>';
	endif;
}
